...
from app.dicts.schemas import (
    DictBaseCode,
    DictBaseListMore, DictCreate, DictBaseModelCode, DictBaseModelCodes
)
...
async def add_item_data(
    ...
    try:
        ...
        # 检查父字典是否存在
        parent_dict = db.query(SYSDict).filter(SYSDict.code == request.code).first()
        if not parent_dict:
            return Message.error(message="父字典不存在", code=ErrorCode.BAD_REQUEST)
        if parent_dict.status == DictStatus.DISABLED.value:
            return Message.error(message="禁用状态的字典不允许添加字典项", code=ErrorCode.BAD_REQUEST)
            
        # 检查同一父字典下是否有重复的 name 或 key
        exists = db.query(SYSDictItem).filter(
            SYSDictItem.parent_code == request.code,
            (SYSDictItem.name == request.name) | (SYSDictItem.key == request.key)
        ).first()
        if exists:
            if exists.name == request.name:
                return Message.error(message="同一字典下已存在相同名称的字典项", code=ErrorCode.BAD_REQUEST)
            else:
                return Message.error(message="同一字典下已存在相同key的字典项", code=ErrorCode.BAD_REQUEST)
            
        # 生成字典项编码
        item_code = sysHex4randCode(prefix="DICTITEM_", length=8)
