...
class RedisDB:
    ...
    def get_user_info(self, account: str, login_type: int) -> dict:
        ...
        try:
            ...
            for k, v in data.items():
                try:
                    result[k] = type_conversion[k](v) if k in type_conversion else v
                except (ValueError, TypeError):
                    result[k] = v  # 如果转换失败，保留原始值
            return result
        except redis.RedisError:
            return None
    ...
    def clear_login_record(self, user_id: int):
        """
        清除用户登录记录
        :param user_id: 用户ID
        """
        try:
            key = f"{REDIS_KEYS['USER_LOGIN']}{user_id}"
            date_set_key = f"{key}:dates"
            self.redis_client.delete(key, date_set_key)
            return True
        except redis.RedisError:
            return False

    def clear_token_cache(self, user_id: int):
        """
        清除用户token缓存
        :param user_id: 用户ID
        """
        try:
            key = f"{REDIS_KEYS['USER_TEMP']}token:{user_id}"
            self.redis_client.delete(key)
            return True
        except redis.RedisError:
            return False
